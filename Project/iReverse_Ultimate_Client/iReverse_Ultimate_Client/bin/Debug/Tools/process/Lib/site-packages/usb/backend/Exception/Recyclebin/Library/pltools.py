...
from Recyclebin.Library.utils import LogBase, print_progress, logsetup
from Recyclebin.Library.hwcrypto import crypto_setup, hwcrypto
from Recyclebin.Library.kamakiri import Kamakiri
    ...
    def runpayload(self, filename, offset=0, ack=0xA1A2A3A4, addr=None, dontack=False):
        ...
        try:
            with open(filename, "rb") as rf:
                rf.seek(offset)
                payload = rf.read()
                self.info(f"Loading payload: {os.path.basename(filename)}, {hex(len(payload))} bytes")
