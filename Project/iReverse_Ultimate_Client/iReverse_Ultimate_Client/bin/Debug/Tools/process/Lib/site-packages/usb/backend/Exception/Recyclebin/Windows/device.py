from Windows.common import to_bytes, from_bytes
from Windows.logger import log
import usb
...
class Device:
    ...
    def find(self, wait=False):
        ...
        try:
            self.backend = usb.backend.libusb1.get_backend(find_library=lambda x: "windows/libusb-1.0.dll")
            if self.backend:
                try:
        ...
        log("Found device: {0:04x}x{1:04x}".format(self.udev.idVendor, self.udev.idProduct))
        self.dev = self
