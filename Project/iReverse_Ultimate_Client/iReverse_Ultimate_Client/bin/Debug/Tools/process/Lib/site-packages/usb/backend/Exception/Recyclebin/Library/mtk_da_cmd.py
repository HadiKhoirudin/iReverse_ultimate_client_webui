...
from Recyclebin.Library.utils import LogBase, logsetup, getint
from Recyclebin.config.payloads import pathconfig
from Recyclebin.Library.error import ErrorHandler
from Recyclebin.Library.utils import progress
...
class DA_handler(metaclass=LogBase):
    ...
    def configure_da(self, mtk, preloader):
        ...
        if mtk.config.target_config is None:
            self.info("Please disconnect, start Recyclebin and reconnect.")
            return None
    ...
    def da_write(self, parttype: str, filenames: list, partitions: list):
        ...
        if parttype == "user" or parttype is None:
            i = 0
            self.info("Checking available partitions ...")
            for partition in partitions:
                partfilename = filenames[i]
                i += 1
                if partition == "gpt":
                    self.info(f"Writing partition: {partfilename}")
