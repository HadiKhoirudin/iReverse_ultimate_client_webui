...
from Recyclebin.Library.utils import LogBase, progress, logsetup
from Recyclebin.Library.error import ErrorHandler
from Recyclebin.Library.daconfig import EMMC_PartitionType, UFS_PartitionType, DaStorage
from Recyclebin.Library.partition import Partition
from Recyclebin.config.payloads import pathconfig
from Recyclebin.Library.xflash_ext import xflashext, XCmd
from Recyclebin.Library.settings import hwparam
...
class DAXFlash(metaclass=LogBase):
    ...
    def formatflash(self, addr, length, storage=None,
        ...
        self.info(f"Formatting addr: {hex(addr)} with length: {hex(length)}")
    ...
    def upload(self):
        ...
        self.info(f"Uploading xflash: {os.path.basename(loader)}")
